...
class ploader extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
            ...
            'exist_url' => array('type' => self::TYPE_INT, 'required' => false),
    ...
    public function create_product($id_load = null)
    {
        if($this->id_pproduct == 0) {
            ...
            if ($id_load) {
                ...
                if($this->getErrors())
                    die('SKIPED: no category or manufacturer or noactive for '.$this->id_load.'<br>');
    ...
    public static function productExist($url, $ex=0){
	    if($ex>0)
        {
            $sql = "update " . _DB_PREFIX_ . "egploader SET exist_url = 1 where url = '".$url."'";
